...
const selfSchema = z.object({
  buyFor: z.literal("self"),
  payerPhone: phoneSchema,
});

const otherSchema = z.object({
  buyFor: z.literal("other"),
  payerPhone: phoneSchema,
...
export function PaymentModal({ offer, open, onOpenChange }: PaymentModalProps) {
  ...
  function switchBuyFor(mode: "self" | "other") {
    setBuyFor(mode);
    if (mode === "self") {
      form.reset({ buyFor: "self", payerPhone: form.getValues("payerPhone") });
    } else {
      form.reset({ buyFor: "other", payerPhone: form.getValues("payerPhone"), recipientPhone: "" });
    }
  }
      ...
      { data: { phone: values.payerPhone, offerId: offer.id, amount: offer.price, recipientPhone: recipientPhone ?? null } },
      {
        onSuccess: (data) => {
          if (data.success && data.checkoutRequestId) {
            ...
            form.setError("root" as never, { message: data.message ?? "Failed to initiate payment" });
          }
        },
        onError: (err: unknown) => {
  ...
  const isFailed =
    ...
    isStatusError ||
    (!!checkoutRequestId && countdown === 0 && !isSuccess);
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-white/[0.06]">
          ...
          <div className="text-right shrink-0">
            ...
            <p className="text-lg font-black text-white leading-tight">
              <span className="text-xs font-bold text-slate-400 mr-0.5">KSH</span>
              {offer.price}
            </p>
          </div>
        </div>
            ...
            <>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
                  ...
                  {/* Recipient phone — only shown when buying for others */}
                  {buyFor === "other" && (
                    <div className="space-y-1.5">
                      <p className="text-[11px] text-slate-400 font-medium">Recipient's Number</p>
                      <p className="text-[10px] text-slate-600 leading-relaxed">
                        The bundle will be sent to this number after payment
                      </p>
                      <FormField
                  ...
                  {offer.category === "data" && !offer.multipleAllowed && (
                    <p className="text-[11px] text-orange-400 font-medium bg-orange-500/8 border border-orange-500/15 px-3 py-2 rounded-lg">
                      ⚠ This bundle can only be purchased once per day per number
                    </p>
                  )}
            ...
            <div className="py-6 flex flex-col items-center text-center space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-slate-500 text-xs leading-relaxed max-w-[210px] mx-auto">
                  An M-Pesa prompt has been sent to your number. Enter your PIN to pay.
                </p>
              </div>
              ...
              <div className="w-full rounded-xl border border-white/[0.07] bg-white/[0.03] divide-y divide-white/[0.05]">
                ...
                {paymentStatus?.recipientPhone && (
                  <div className="flex justify-between items-center px-4 py-2.5">
                    <span className="text-xs text-slate-500">Recipient</span>
                    <span className="text-xs font-bold text-white">{paymentStatus.recipientPhone}</span>
                  </div>
                )}
              </div>
          ...
          {isFailed && !isSuccess && (
            <div className="py-4 flex flex-col items-center text-center space-y-4">
              ...
              <div className="flex w-full gap-2">
                <Button
                  className="flex-1 h-11 font-bold text-sm rounded-xl border border-white/10 text-slate-400"
